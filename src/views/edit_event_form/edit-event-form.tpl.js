...
const createEditEventFormTemplate = (point, allOffers, allDestinations) => {
  ...
  const createDestinationSectionTemplate = (
    ...
    if (receivedDestination?.name !== null) {
      const destinationWithCityName = destinations.find(
        (currentDestination) =>
          receivedDestination?.name === currentDestination.name
      );
      ...
      const createPicturesForDestinationTemplate = (images) => {
        if (images !== undefined) {
          const imagesForContainer = images
            .map(
              (image) =>
  ...
  const createTypeCheckerTemplate = (typeOfEvent, allTypes) => {
    const typeItemsForFieldset = allTypes
      .map(
        (currentType) =>
  ...
  const createAvailableOffrersTemplate = (
    allAvailableOffrers,
    currentType,
    pointOffers
  ) => {
  ...
  const createAvailableOffrersSectionTemplate = (
    allAvailableOffrers,
    currentType,
    templateWithOffers
  ) => {
